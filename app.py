        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5, smooth_landmarks=True) as pose:
            while self.cap.isOpened():
                ...
                try:
                    ...
                    shouder_center = [(shoulder_left[0] + shoulder_right[0]) / 2,
                                      (shoulder_left[1] + shoulder_right[1]) / 2]

                    nose = [landmarks[mp_pose.PoseLandmark.NOSE.value].x, landmarks[mp_pose.PoseLandmark.NOSE.value].y + 1]

                    angle = self.calculateAngle(nose, shouder_center, shoulder_right)
